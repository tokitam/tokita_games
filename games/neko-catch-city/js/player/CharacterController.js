...
export class CharacterController {
  constructor(world, spawn) {
    ...
    this.root.position.set(spawn.x, 0, spawn.z);
    ...
    // tap-move destination
    this._goalX = spawn.x; this._goalZ = spawn.z; this._goalRun = false; this._hasGoal = false;
  }
  ...
  update(dt, moveX, moveY, run, cameraYaw) {
    // Keyboard input takes priority; tap goal used when no keyboard input
    ...
    const accel = Math.hypot(targetVX, targetVZ) > Math.hypot(this._velX, this._velZ);
    const k = 1 - Math.exp(-dt / (accel ? CONFIG.accelTau : CONFIG.decelTau));
